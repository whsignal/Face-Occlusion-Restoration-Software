...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.label.setMinimumSize(QtCore.QSize(512, 512))                   ##长宽相等。自适应。需要在跑网络之前放大到512*512
        ...
        self.picture_after.setMinimumSize(QtCore.QSize(512, 512))           ##处理后的图像大小设定
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.ui.DoOperate.clicked.connect(self.on_do_operate_clicked)     #处理按键
    ...
    def export_image(self):
        ...
        if self.pixmap and self.drawing_pixmap:
            ...
            print(f"保存图像大小: {final_pixmap.width()}x{final_pixmap.height()}")

            # 将 QPixmap 转换为 OpenCV 格式
            qimg = final_pixmap.toImage()
            qimg = qimg.convertToFormat(QtGui.QImage.Format_RGB32)
            width = qimg.width()
            height = qimg.height()
            ptr = qimg.bits()
            ptr.setsize(qimg.byteCount())
            arr = np.array(ptr).reshape(height, width, 4)  # 4 通道 (R, G, B, A)
            cv_img = cv2.cvtColor(arr, cv2.COLOR_BGRA2BGR)  # 转换为 BGR 格式
    
            # 保存图像到文件
            file_path_out, _ = QFileDialog.getSaveFileName(self, "保存图片", "", "*.png;;*.jpg;;*.bmp")
            if file_path_out:
                if file_path_out.lower().endswith('.png'):
                    # 使用 OpenCV 保存为 PNG，设置压缩级别为 0（无压缩）
                    cv2.imwrite(file_path_out, cv_img, [cv2.IMWRITE_PNG_COMPRESSION, 0])
                elif file_path_out.lower().endswith('.jpg') or file_path_out.lower().endswith('.jpeg'):
                    # 使用 OpenCV 保存为 JPEG，设置质量为 100（最高质量）
                    cv2.imwrite(file_path_out, cv_img, [cv2.IMWRITE_JPEG_QUALITY, 100])
                elif file_path_out.lower().endswith('.bmp'):
                    # BMP 格式本身是无损的，直接保存
                    cv2.imwrite(file_path_out, cv_img)
                print(f"图像已保存到: {file_path_out}")
