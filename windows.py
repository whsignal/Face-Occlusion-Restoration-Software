...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def export_image(self):
        ...
        if self.pixmap and self.drawing_pixmap:
            ...
            # 保存图像到文件
            file_path_out, _ = QFileDialog.getSaveFileName(self, "保存图片", "", "*.bmp")
            if file_path_out:
                final_pixmap.save(file_path_out)
    ...
    def on_do_operate_clicked(self):
        ...
        #self.ui.label.setPixmap(processed_pixmap)
        ...
        # image.save(temp_image_path, "PNG", quality=100)  # 设置保存质量为100以确保无损保存

        # 调用 run_inference 函数处理图片
        run_inference(input_path=self.file_path, output_path=None, suffix=None)

        # 假设处理后的图片保存在 outputs/processed_image.png
        processed_image_path = "outputs/processed_image.png"
    ...
    # def verify_image_size(self, image_path):
        # 使用 OpenCV 读取图片并验证其大小
        # image = cv2.imread(image_path)
        # height, width = image.shape[:2]
        # print(f"文件中保存的图像大小: {width}x{height}")
        # assert (width, height) == (512, 512), "保存的图像大小不符合要求"
